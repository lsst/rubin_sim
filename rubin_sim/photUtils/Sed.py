...
class sed_unpickler(pickle.Unpickler):
    ...
    def find_class(self, module, name):
        ...
        if not allowed:
            raise RuntimeError(
                "Cannot call find_class() on %s, %s with sed_unpickler "
                % (module, name)
...
class Sed(object):
    ...
    def flambdaTofnu(self, wavelen=None, flambda=None):
        ...
        fnu = (
            ...
            * wavelen
            * wavelen
            * self._physParams.nm2m
            / self._physParams.lightspeed
        )
    ...
    def setupCCMab(self, wavelen=None):
        ...
        """
        warnings.warn(
            "Sed.setupCCMab is now deprecated in favor of Sed.setupCCM_ab",
            DeprecationWarning,
        )
    ...
    def addCCMDust(
        ...
        """
        warnings.warn(
            "Sed.addCCMDust is now deprecated in favor of Sed.addDust",
            DeprecationWarning,
        )
        return self.addDust(
    ...
    def multiplySED(self, other_sed, wavelen_step=None):
        ...
        if numpy.all(self.wavelen == other_sed.wavelen):
            ...
            wavelen_1, flambda_1 = self.resampleSED(
                self.wavelen,
                self.flambda,
                wavelen_min=wavelen_min,
                wavelen_max=wavelen_max,
            ...
            wavelen_2, flambda_2 = self.resampleSED(
    ...
    def calcErgs(self, bandpass):
        ...
        """
        wavelen, flambda = self.resampleSED(
...
def read_close_Kurucz(teff, feH, logg):
    ...
    if not hasattr(read_close_Kurucz, "param_combos"):
        ...
        read_close_Kurucz.param_combos = numpy.zeros(
            len(kurucz_files),
            dtype=[
                ("filename", ("|U200")),
                ("teff", float),
        ...
        read_close_Kurucz.param_combos = numpy.sort(
            read_close_Kurucz.param_combos, order=["teff", "feH", "logg"]
        )
