        ...
        for i in range(1, len(dicts)):
            ...
            for key in dict_of_lists:
                if isinstance(dicts[i][key][new_mjds], list):
                    ...
                    dict_of_lists[key] = np.concatenate(
                        (dict_of_lists[key], dicts[i][key][new_mjds])
                    )
...
def generate_percentiles(nbins=20):
    ...
    np.savez(
        "percentile_m5_maps.npz", histograms=histograms, histogram_npts=histogram_npts
    )
