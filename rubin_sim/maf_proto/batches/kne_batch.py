...
def kne_batch(observations=None, run_name=None, quick_test=False, fig_saver=None):
    ...
    sl = maf.Slicer(nside=None, missing=0,
                    ra=np.degrees(metric.ra),
                    dec=np.degrees(metric.dec))
    ...
    for key in kn_array.dtype.names:
        ...
        summary_stats.append(maf.gen_summary_row(info, 'Uncert in Mean KNe ' + key,
                                                 np.mean(kn_array[key])/np.sqrt(np.sum(kn_array[key]))))

    fig = ph(np.degrees(metric.ra), np.degrees(metric.dec), 
             kn_array["multi_detect"], unit="KNe mean multi_detect")
