...
class MicrolensingMetric(BaseMetric):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(unit=self.units, name=name + "_" + self.metric_calc, **kwargs)
    ...
    def __call__(self, visits, slice_point=None):
        ...
        amplitudes = microlensing_amplification(
            ...
            crossing_time=self.events["crossing_time"][indx],
            peak_time=self.events["peak_time"][indx],
        )
        # individual_mags = False

        filters = np.unique(visits[self.filter_col])
        ...
        snr = m52snr(amplified_mags, visits[self.m5_col])
            ...
            if self.metric_calc == "detect":
                if self.time_before_peak == "optimal":
                    ...
                    infilt = np.where(
                        ...
                        & (t < (self.events["peak_time"][indx] - time_before_peak_optimal))
                    )[0]
                    ...
                    infilt = np.where(
                        (visits[self.filter_col] == filtername)
                        & (t < (self.events["peak_time"][indx] - self.time_before_peak))
                    )[0]

                # observations post-peak and in the given filter
                outfilt = np.where(
                    (visits[self.filter_col] == filtername) & (t > self.events["peak_time"][indx])
                )[0]
    ...
    def generate_microlensing_events(
        self,
        min_crossing_time=1,
        ...
        """Generate a population of microlensing events.
        ...
        self.ra, self.dec = c.ra.rad, c.dec.rad

        self.events = {}
        self.events["peak_time"] = peak_times
        self.events["crossing_time"] = crossing_times
        self.events["impact_parameter"] = impact_paramters
