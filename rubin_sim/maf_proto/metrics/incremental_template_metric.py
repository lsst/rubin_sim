    ...
    def __init__(
        ...
        **kwargs,
        ...
        names = [
            "bench_seeing",
            "bench_m5",
            "night_template_created",
            "frac_into_season_template_created",
            "n_images_until_template",
            "n_images_with_template",
            "template_m5",
            "template_seeing",
            "total_alerts",
            "template_input_m5s",
            "template_input_seeing",
            "fraction_better_template_seeing",
        ]
        types = [float, float, int, float, int, int, float, float, int, float, float, float]

        self.result = np.zeros(1, dtype=list(zip(names, types)))

        self.badval = copy(self.result)
        for key, type in zip(names, types):
            if isinstance(type, float):
                self.badval[key] = badval

    def __call__(self, data_slice, slice_point):
