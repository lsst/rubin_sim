...
class CheckColorSlope(object):
    ...
    def __call__(self, visits):
        has_color = False
        has_slope = False

        if np.size(visits) < 3:
            return 0
        filters = visits[self.filter_col]
...
class ColorSlopeMetric(BaseMetric):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(unit=unit, name=name, **kwargs)
    ...
    def __call__(self, visits, slice_point=None):
        result = 0
        deep_enough = np.zeros(visits.size, dtype=bool)
        for filtername in np.unique(visits[self.filter_col]):
            in_filt = np.where(visits[self.filter_col] == filtername)[0]
            indx = np.where(visits[self.m5_col][in_filt] > self.mag[filtername])[0]
            deep_enough[in_filt[indx]] = True
        data = visits[deep_enough]
        if data.size > 0:
