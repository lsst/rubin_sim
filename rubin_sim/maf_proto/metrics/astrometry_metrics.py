...
from rubin_sim.maf_proto.utils import astrom_precision, m52snr, sigma_slope
...
class ParallaxMetric(BaseMetric):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if normalize:
            unit = "Parallax Uncertainty (ratio)"
        else:
            unit = "Parallax Uncertainty (mas)"
    ...
    def __call__(self, data_slice, slice_point=None):
        ...
        for filt in filters:
            ...
            snr[good] = m52snr(self.mags[str(filt)], data_slice[self.m5_col][good])
    ...
    def __call__(self, data_slice, slice_point=None):
        ...
        for f in filters:
            ...
            if np.size(observations[0]) < 2:
                precis[observations] = maskval
            else:
                snr = m52snr(self.mags[f], data_slice[self.m5_col][observations])
                precis[observations] = astrom_precision(
                    data_slice[self.seeing_col][observations], snr, self.atm_err
                )
        good = np.where(precis != maskval)
        result = sigma_slope(data_slice[self.mjd_col][good], precis[good])
...
class ParallaxCoverageMetric(BaseMetric):
    ...
    def __call__(self, data_slice, slice_point=None):
        ...
        for filt in filters:
            ...
            snr[in_filt] = m52snr(self.mags[str(filt)], data_slice[self.m5_col][in_filt])
...
class ParallaxDcrDegenMetric(BaseMetric):
    ...
    def __call__(self, data_slice, slice_point=None):
        ...
        for filt in np.unique(data_slice[self.filter_col]):
            in_filt = np.where(data_slice[self.filter_col] == filt)
            snr[in_filt] = m52snr(self.mags[filt], data_slice[self.m5_col][in_filt])
