...
class XRBPopMetric(BaseMetric):
    ...
    def __init__(
        ...
        names = [
            ...
            "mean_time_between_detections",
        ]

        self.dtype = list(zip(names, [float] * len(names)))
        self.empty = np.empty(1, dtype=self.dtype)
        self.badval = np.zeros(1, dtype=self.dtype)
        self.badval["mean_time_between_detections"] = np.nan
    ...
    def _early_detect(self, where_detected, time, early_window_days=7.0, n_early_detections=2):
        ...
        mags : `array`
            magnitudes obtained interpolating models on the visits
        time : `array`
            relative times
    ...
    def __call__(self, visits, slice_point=None):
        ...
        t = visits[self.mjd_col] - self.mjd0 - self.events["start_time"][indx]
        mags = np.zeros(t.size, dtype=float)

        for filtername in np.unique(visits[self.filter_col]):
            infilt = np.where(visits[self.filter_col] == filtername)
            mags[infilt] = self.lightcurves.lightcurve(
                t[infilt], filtername, self.events["outburst_params"][indx]
            )
            # Apply dust extinction on the light curve
            extinction = eb_v_hp(self.dust_nside, ra=self.ra[indx], dec=self.dec[indx])
            a_x = self.ax1[filtername] * extinction
            mags[infilt] += a_x

            distmod = 5 * np.log10(self.events["distance"][indx] * 1.0e3) - 5.0
            mags[infilt] += distmod

        # Find the detected points
        where_detected = np.where((mags < visits[self.m5_col]))[0]
        # Magnitude uncertainties with Gaussian approximation
        snr = m52snr(mags, visits[self.m5_col])
        mags_unc = 2.5 * np.log10(1.0 + 1.0 / snr)

        result["possible_to_detect"] = self._possible_to_detect(self.events["visible_duration"][indx])
        ...
        if result["number_of_detections"] > 1:
            result["mean_time_between_detections"] = self._mean_time_between_detections(
                [
                    self.events["visible_start_time"][indx],
                    *t[where_detected].tolist(),
                    self.events["visible_end_time"][indx],
                ]
            )
        else:
            result["mean_time_between_detections"] = np.nan

        # Export the light curve
        if self.output_lc is True:
            wdet = mags < visits[self.m5_col]
            mags[~wdet] = 99.0
            result["lc"] = [
                visits[self.mjd_col],
                mags,
                mags_unc,
                visits[self.m5_col],
                visits[self.filter_col],
            ]
            result["lc_colnames"] = ("t", "mag", "mag_unc", "maglim", "filter")

        return result

    def generate_xrb_pop(self, t_start=1, t_end=3652, n_events=10000, seed=42):
        ...
        self.ra = np.radians(xrb_sample[event_idxs, 0])
        self.dec = np.radians(xrb_sample[event_idxs, 1])
        ...
        self.events["start_time"] = start_times
        self.events["distance"] = distance_kpc
        # use our own 3-d dust map extinctions
        self.events["ebv"] = ebv
        # generate random parameters for this event
        self.events["outburst_params"] = xrb_lc_gen.outburst_params(size=n_events)

        # determine detectable durations
        visible_starts = []
        visible_ends = []
        visible_durations = []
        for idx, param in enumerate(self.events["outburst_params"]):
        ...
        self.events["visible_start_time"] = visible_starts
        self.events["visible_end_time"] = visible_ends
        self.events["visible_duration"] = visible_durations
