...
class OneDSlicer(BaseSlicer):
    ...
    def __init__(
        ...
        if sliceColName is None:
            raise ValueError(
                "sliceColName cannot be left None - choose a data column to group data by"
            )
        ...
        if self.bins is not None:
            if binMin is not None or binMax is not None or binsize is not None:
                warnings.warning(
                    f"Both bins and one of the binMin/binMax/binsize was specified. "
            ...
            if self.sliceColName == otherSlicer.sliceColName:
                ...
                # However, before we 'setup' the slicer with data, the slicers could be equivalent.
                    ...
                    if (self.bins is not None) and (otherSlicer.bins is not None):
                        ...
                        and (self.binMin is not None) & (self.binMax is not None)
                        and (otherSlicer.binsize is not None)
                        and (otherSlicer.binMin is not None)
                        and (otherSlicer.binMax is not None)
                    ):
                        if (
                            (self.binsize == otherSlicer.binsize)
                            and (self.binMin == otherSlicer.binMin)
                            and (self.binMax == otherSlicer.binMax)
                        ):
                            result = True
        return result
