...
class LCfast:
    ...
    def processBand(self, sel_obs, band, gen_par, j=-1, output_q=None):
        ...
        fluxes_obs[fluxes_obs <= 0.0] = 1.0e-10
    ...
    def __init__(
        ...
        for band in bands:
            ...
            self.flux[band] = RegularGridInterpolator(
                (phav, zv), flux, method=method, bounds_error=False, fill_value=0.0
            )
            self.fluxerr[band] = RegularGridInterpolator(
                (phav, zv), fluxerr, method=method, bounds_error=False, fill_value=0.0
            )
