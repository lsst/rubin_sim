        ...
        if childMetrics is None:
            try:
                if not isinstance(self.childMetrics, dict):
                    raise ValueError(
                        "self.childMetrics must be a dictionary (possibly empty)"
                    )
...
class DiscoveryMetric(BaseMoMetric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.snrLimit = snrLimit
        self.childMetrics = {
            "N_Chances": Discovery_N_ChancesMetric(self),
        ...
        if "metricName" in kwargs:
            ...
            metricName = "Discovery_%.0fx%.0fin%.0f" % (
                nObsPerNight,
                nNightsPerWindow,
                tWindow,
            )
        ...
        super().__init__(
            ...
            childMetrics=self.childMetrics,
            badval=badval,
            **kwargs
        )
    ...
    def run(self, ssoObs, orb, Hval):
        ...
        good = np.where(
            (timesEnd - timesStart >= self.tMin) & (timesEnd - timesStart <= self.tMax),
            1,
            0,
        )
        ...
        check = np.where(
            ...
            & (timesEnd - timesStart > self.tMax)
        )[0]
        for i, j, c in zip(
            visSort[nIdxMany][check], visSort[nIdxManyEnd][check], check
        ):
        ...
        if len(goodIdx) < self.nNightsPerWindow:
            return self.badval
        deltaNights = (
            np.roll(ssoObs[self.nightCol][vis][goodIdx], 1 - self.nNightsPerWindow)
        ...
        for i, sIdx in enumerate(startIdxs):
            inWindow = np.where(
                ssoObs[self.nightCol][vis][goodIdx]
                - ssoObs[self.nightCol][vis][goodIdx][sIdx]
                <= self.tWindow
            )[0]
        ...
        return {
            "start": startIdxs,
            "end": endIdxs,
            "trackletNights": ssoObs[self.nightCol][vis][goodIdx],
        }
...
class Discovery_N_ChancesMetric(BaseChildMetric):
    ...
    def __init__(
        self, parentDiscoveryMetric, nightStart=None, nightEnd=None, badval=0, **kwargs
    ):
    ...
    def run(self, ssoObs, orb, Hval, metricValues):
        ...
        if self.nightEnd is None and self.nightStart is not None:
            ...
            valid = np.where(
                (startNights >= self.nightStart) & (endNights <= self.nightEnd)
            )[0]
        return len(valid)
...
class Discovery_DistanceMetric(BaseChildMetric):
    ...
    def __init__(
        self, parentDiscoveryMetric, i=0, distanceCol="geo_dist", badval=-999, **kwargs
    ):
        super().__init__(parentDiscoveryMetric, badval=badval, **kwargs)
        self.i = i
...
class ActivityOverTimeMetric(BaseMoMetric):
    ...
    def __init__(self, window, snrLimit=5, surveyYears=10.0, metricName=None, **kwargs):
        ...
        self.windowBins = np.arange(
            0, self.surveyYears * 365 + self.window / 2.0, self.window
        )
...
class ActivityOverPeriodMetric(BaseMoMetric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        if metricName is None:
            metricName = "Chance of detecting activity covering %.1f of the orbit" % (
                binsize
            )
        super().__init__(metricName=metricName, **kwargs)
        self.qCol = qCol
    ...
    def run(self, ssoObs, orb, Hval):
        ...
        if self.anomalyCol in orb.keys():
            curranomaly = np.radians(
                orb[self.anomalyCol]
                + (ssoObs[self.mjdCol] - orb["epoch"]) / period * 360.0
            ) % (2 * np.pi)
        elif self.tPeriCol in orb.keys():
            curranomaly = ((ssoObs[self.mjdCol] - orb[self.tPeriCol]) / period) % (
                2 * np.pi
            )
        else:
            return self.badval
...
class HighVelocityMetric(BaseMoMetric):
    ...
    def run(self, ssoObs, orb, Hval):
        ...
        if len(vis) == 0:
            return self.badval
        highVelocityObs = np.where(
            ssoObs[self.velocityCol][vis]
            >= (
                24.0
                * self.psfFactor
                * ssoObs[self.seeingCol][vis]
                / ssoObs[self.expTimeCol][vis]
            )
        )[0]
...
class HighVelocityNightsMetric(BaseMoMetric):
    ...
    def run(self, ssoObs, orb, Hval):
        ...
        nWithXObs = n[np.where(obsPerNight >= self.nObsPerNight)]
        if len(nWithXObs) > 0:
            found = ssoObs[np.where(ssoObs[self.nightCol] == nWithXObs[0])][
                self.mjdCol
            ][0]
        else:
            found = self.badval
...
class Color_AsteroidMetric(BaseMoMetric):
    ...
    def run(self, ssoObs, orb, Hval):
        ...
        if len(filterWeight) == 0:  # this lets us stop evaluating here if possible.
            ...
            if ("r" in filterWeight or "i" in filterWeight) and (
                "z" in filterWeight or "y" in filterWeight
            ):
                flag = 1
            else:
                flag = 0
...
class KnownObjectsMetric(BaseMoMetric):
    ...
    def run(self, ssoObs, orb, Hval):
        ...
        if len(overPeak) > 0:
            discoveryTime = self._pickObs(
                ssoObs[self.mjdCol][obs1][overPeak], self.eff1
            )
        # Second period.
        if discoveryTime is None:
            obs2 = np.where(
                (ssoObs[self.mjdCol] >= self.tSwitch1)
                & (ssoObs[self.mjdCol] < self.tSwitch2)
                & visible
            )[0]
            overPeak = np.where(ssoObs[self.appMagVCol][obs2] <= self.vMagThresh2)[0]
            if len(overPeak) > 0:
                discoveryTime = self._pickObs(
                    ssoObs[self.mjdCol][obs2][overPeak], self.eff2
                )
        # Third period.
        if discoveryTime is None:
            obs3 = np.where(
                (ssoObs[self.mjdCol] >= self.tSwitch2)
                & (ssoObs[self.mjdCol] < self.tSwitch3)
                & visible
            )[0]
            overPeak = np.where(ssoObs[self.appMagVCol][obs3] <= self.vMagThresh3)[0]
            if len(overPeak) > 0:
                discoveryTime = self._pickObs(
                    ssoObs[self.mjdCol][obs3][overPeak], self.eff3
                )
        # Fourth period.
        if discoveryTime is None:
            ...
            if len(overPeak) > 0:
                discoveryTime = self._pickObs(
                    ssoObs[self.mjdCol][obs4][overPeak], self.eff4
                )
        if discoveryTime is None:
