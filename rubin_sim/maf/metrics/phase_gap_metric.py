...
class PhaseGapMetric(BaseMetric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(PhaseGapMetric, self).__init__(
            col, metricName=metricName, units="Fraction, 0-1", **kwargs
        )
    ...
    def run(self, dataSlice, slicePoint=None):
        ...
        if step == 0:
            ...
            periods = (
                periods / np.max(periods) * (self.periodMax - self.periodMin)
                + self.periodMin
            )
    ...
    def reduceWorstPeriod(self, metricVal):
        ...
        """
        worstP = metricVal["periods"][
            np.where(metricVal["maxGaps"] == metricVal["maxGaps"].max())
        ]
        return worstP
...
class PeriodicQualityMetric(BaseMetric):
    def __init__(
        ...
        metricName="PhaseCoverageMetric",
        starMag=20,
        **kwargs
    ):
        self.mjdCol = mjdCol
        ...
        super(PeriodicQualityMetric, self).__init__(
            [mjdCol, m5Col], metricName=metricName, units="Fraction, 0-1", **kwargs
        )
    ...
    def _calc_amp(self, dataSlice):
        ...
        amp_snrs = (
            np.sin(
                dataSlice[self.mjdCol] / self.period * 2 * np.pi + phases[:, np.newaxis]
            )
            * snr
        )
