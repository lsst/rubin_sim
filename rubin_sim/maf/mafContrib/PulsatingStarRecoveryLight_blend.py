...
__all__ = ['PulsatingStarRecoveryLight']

class PulsatingStarRecoveryLight(maf.BaseMetric):
    ...
    ----------
     ...
     factorForDimensionGap: float
        ...
        airmass=airmass 
        test_ra= ra of the center of the slice for the TRILEGAL search
        test_dec =dec of the center of the slice for the TRILEGAL search
        
        
       
    """
    def __init__(self,filename,dmod,sigmaFORnoise,do_remove_saturated,numberOfHarmonics,factorForDimensionGap,test_ra, test_dec,mjdCol='observationStartMJD',fiveSigmaDepth='fiveSigmaDepth',filters= 'filter', night='night',visitExposureTime='visitExposureTime',skyBrightness='skyBrightness', numExposures='numExposures', seeing='seeingFwhmEff',airmass='airmass',**kwargs):
        ...
        cols = [self.mjdCol,self.fiveSigmaDepth,self.filters,self.night,self.visitExposureTime,self.skyBrightness,self.numExposures,self.seeing,self.airmass] # Add any columns that your metric needs to run -- mjdCol is just an example
        ...
        super().__init__(col=cols, maps=maps, units='#', **kwargs, metricName="PulsatingStarRecoveryL", metricDtype="object")
        

        
    def run(self, dataSlice, slicePoint=None):
        
        #Here we build df from trilegal that will be used by ReadTeoSim_blend which is the list of stars and their apparent magnitudes
        import dl.queryClient as qc
        test_ra = self.test_ra
        ...
        #This is a control.If df is empty (no blend) use ReadTeoSim
        if df.empty:
            lcModel=self.ReadTeoSim(self.lcModel_ascii,self.dmod,ebv1)
            lcModel_noblend={'amplu':lcModel['amplu'],'amplg':lcModel['amplg'],'amplr':lcModel['amplr'],'ampli':lcModel['ampli'],'amplz':lcModel['amplz'],'amply':lcModel['amply'],'meanu':lcModel['meanu'],'meang':lcModel['meang'],'meanr':lcModel['meanr'],'meani':lcModel['meani'],'meanz':lcModel['meanz'],'meany':lcModel['meany']}
        else:
            lcModel=self.ReadTeoSim_blend(df,self.lcModel_ascii,self.dmod,ebv1)
        ...
        deltaAmp_u=lcModel_noblend['amplu']-lcModel['amplu']
        deltaAmp_g=lcModel_noblend['amplg']-lcModel['amplg']
        deltaAmp_r=lcModel_noblend['amplr']-lcModel['amplr']
        deltaAmp_i=lcModel_noblend['ampli']-lcModel['ampli']
        deltaAmp_z=lcModel_noblend['amplz']-lcModel['amplz']
        deltaAmp_y=lcModel_noblend['amply']-lcModel['amply']
        
        deltamean_u=lcModel_noblend['meanu']-lcModel['meanu']
        deltamean_g=lcModel_noblend['meang']-lcModel['meang']
        deltamean_r=lcModel_noblend['meanr']-lcModel['meanr']
        deltamean_i=lcModel_noblend['meani']-lcModel['meani']
        deltamean_z=lcModel_noblend['meanz']-lcModel['meanz']
        deltamean_y=lcModel_noblend['meany']-lcModel['meany']
        
        ampg_noblend=lcModel_noblend['amplg']
        mediag=lcModel_noblend['meang']
        mediag_blend=lcModel['meang']
        print('amp_noblen-amp_blend (ampg_noblend=', ampg_noblend)
        print(deltaAmp_u)
        print(deltaAmp_g)
        print(deltaAmp_r)
        print(deltaAmp_i)
        print(deltaAmp_z)
        print(deltaAmp_y)
        print('meanmag_noblen-meanmmag_blend (ampg_noblend=', ampg_noblend)
        print(deltamean_u)
        print(deltamean_g)
        print(deltamean_r)
        print(deltamean_i)
        ...
        LcTeoLSST=self.generateLC(time_lsst,filters_lsst,lcModel)
        
        #the function 'noising' compute and add errors
        snr=self.retrieveSnR(mv,lcModel) 
...
#period=best_per_temp #or period_model or fitLS_multi.best_period
        ...
        output_metric={'n_u':uni_meas['n_u'],'n_g':uni_meas['n_g'],'n_r':uni_meas['n_r'],'n_i':uni_meas['n_i'],'n_z':uni_meas['n_z'],'n_y':uni_meas['n_y'],
                ...
                'uniformity_i':uni_meas['numberGaps_u'],'uniformity_z':uni_meas['numberGaps_u'],'uniformity_y':uni_meas['numberGaps_u'],
...
#    def reduceDelta_Period(self, metricValue):
#        return metricValue['Delta_Period']
  
#    def reduceDelta_Period_abs_cicli(self, metricValue):
#        return metricValue['Delta_Period_abs_cicli']
    
    def reducedeltamag_r(self, metricValue):
        return metricValue['deltamag_r']
    def reducedeltaamp_r(self, metricValue):
