    ...
    def _generateRepRandomOffsets(self, noffsets, numTiles):
        ...
        if self.printInfo:
            ...
            print(
                ...
                float(noffsets) / numPointsInsideHex,
            )
        ...
        xCenter_copy = list(
            np.array(xCenter).copy()
        )  # in case need to reuse the squares
        yCenter_copy = list(
            np.array(yCenter).copy()
        )  # in case need to reuse the squares
    ...
    def _run(self, simData):
        ...
        for fieldid in np.unique(simData[self.fieldIdCol]):
            ...
            simData["repulsiveRandomDitherFieldPerVisitRa"][match] = simData[
                self.raCol
            ][match] + self.xOff / np.cos(simData[self.decCol][match])
            simData["repulsiveRandomDitherFieldPerVisitDec"][match] = (
                simData[self.decCol][match] + self.yOff
            )

        # Wrap back into expected range.
        (
            simData["repulsiveRandomDitherFieldPerVisitRa"],
            simData["repulsiveRandomDitherFieldPerVisitDec"],
        ) = wrapRADec(
            simData["repulsiveRandomDitherFieldPerVisitRa"],
            simData["repulsiveRandomDitherFieldPerVisitDec"],
        )
...
class RepulsiveRandomDitherFieldPerNightStacker(
    ...
    def _run(self, simData):
        ...
        for fieldid in np.unique(simData[self.fieldIdCol]):
            ...
            simData["repulsiveRandomDitherFieldPerNightRa"][match] = simData[
                self.raCol
            ][match] + self.xOff[vertexIdxs] / np.cos(simData[self.decCol][match])
            simData["repulsiveRandomDitherFieldPerNightDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexIdxs]
            )
        # Wrap into expected range.
        (
            simData["repulsiveRandomDitherFieldPerNightRa"],
...
class RepulsiveRandomDitherPerNightStacker(RepulsiveRandomDitherFieldPerVisitStacker):
    ...
    def _run(self, simData):
        ...
        for n, x, y in zip(nights, self.xOff, self.yOff):
            ...
            simData["repulsiveRandomDitherPerNightRa"][match] = simData[self.raCol][
                match
            ] + x / np.cos(simData[self.decCol][match])
            simData["repulsiveRandomDitherPerNightDec"][match] = (
                simData[self.decCol][match] + y
            )
        # Wrap RA/Dec into expected range.
        (
            simData["repulsiveRandomDitherPerNightRa"],
...
class FermatSpiralDitherFieldPerVisitStacker(BaseStacker):
    ...
    def _run(self, simData):
        ...
        for fieldid in np.unique(simData[self.fieldIdCol]):
            ...
            simData["fermatSpiralDitherFieldPerVisitRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexIdxs] / np.cos(simData[self.decCol][match])
            simData["fermatSpiralDitherFieldPerVisitDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexIdxs]
            )
        # Wrap into expected range.
        (
            simData["fermatSpiralDitherFieldPerVisitRa"],
...
class FermatSpiralDitherFieldPerNightStacker(FermatSpiralDitherFieldPerVisitStacker):
    ...
    def _run(self, simData):
        ...
        for fieldid in np.unique(simData[self.fieldIdCol]):
            ...
            vertexIdxs = np.searchsorted(np.unique(nights), nights)
            vertexIdxs = vertexIdxs % self.numPoints
            simData["fermatSpiralDitherFieldPerNightRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexIdxs] / np.cos(simData[self.decCol][match])
            simData["fermatSpiralDitherFieldPerNightDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexIdxs]
            )
        # Wrap into expected range.
        (
            simData["fermatSpiralDitherFieldPerNightRa"],
            simData["fermatSpiralDitherFieldPerNightDec"],
        ) = wrapRADec(
            simData["fermatSpiralDitherFieldPerNightRa"],
            simData["fermatSpiralDitherFieldPerNightDec"],
        )
...
class FermatSpiralDitherPerNightStacker(FermatSpiralDitherFieldPerVisitStacker):
    ...
    def _run(self, simData):
        ...
        for n in nights:
            ...
            simData["fermatSpiralDitherPerNightRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexID] / np.cos(simData[self.decCol][match])
            simData["fermatSpiralDitherPerNightDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexID]
            )
            vertexID += 1

        # Wrap into expected range.
        (
            simData["fermatSpiralDitherPerNightRa"],
...
class PentagonDitherFieldPerSeasonStacker(BaseStacker):
    ...
    def _run(self, simData):
        ...
        seasons = calc_season(simData[self.raCol], simdata[self.expMJDCol])
        ...
        for fieldid in np.unique(simData[self.fieldIdCol]):
            ...
            vertexIdxs = vertexIdxs % len(self.xOff)
            simData["pentagonDitherFieldPerSeasonRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexIdxs] / np.cos(simData[self.decCol][match])
            simData["pentagonDitherFieldPerSeasonDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexIdxs]
            )
        # Wrap into expected range.
        (
            simData["pentagonDitherFieldPerSeasonRa"],
...
class PentagonDiamondDitherFieldPerSeasonStacker(BaseStacker):
    ...
    def _generateOffsets(self):
        ...
        self.xOff = np.concatenate(
            ([0], zip(*diamondCoord)[0], zip(*pentCoord)[0]), axis=0
        )
        self.yOff = np.concatenate(
            ([0], zip(*diamondCoord)[1], zip(*pentCoord)[1]), axis=0
        )
    ...
    def _run(self, simData):
        ...
        for fieldid in np.unique(simData[self.fieldIdCol]):
            ...
            vertexIdxs = np.searchsorted(np.unique(seasonsVisited), seasonsVisited)
            vertexIdxs = vertexIdxs % len(self.xOff)
            simData["pentagonDiamondDitherFieldPerSeasonRa"][match] = simData[
                self.raCol
            ][match] + self.xOff[vertexIdxs] / np.cos(simData[self.decCol][match])
            simData["pentagonDiamondDitherFieldPerSeasonDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexIdxs]
            )
        # Wrap into expected range.
        (
            simData["pentagonDiamondDitherFieldPerSeasonRa"],
...
class PentagonDitherPerSeasonStacker(PentagonDitherFieldPerSeasonStacker):
    ...
    def _run(self, simData):
        ...
        for s in np.unique(seasons):
            ...
            simData["pentagonDitherPerSeasonRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexID] / np.cos(simData[self.decCol][match])
            simData["pentagonDitherPerSeasonDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexID]
            )
            vertexID += 1
        ...
        # Wrap into expected range.
        (
            simData["pentagonDitherPerSeasonRa"],
...
class PentagonDiamondDitherPerSeasonStacker(PentagonDiamondDitherFieldPerSeasonStacker):
    ...
    def _run(self, simData):
        ...
        for s in uniqSeasons:
            match = np.where(seasons == s)[0]
            vertexID = vertexID % len(self.xOff)
            simData["pentagonDiamondDitherPerSeasonRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexID] / np.cos(simData[self.decCol][match])
            simData["pentagonDiamondDitherPerSeasonDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexID]
            )
            vertexID += 1
        ...
        # Wrap into expected range.
        (
            simData["pentagonDiamondDitherPerSeasonRa"],
...
class SpiralDitherPerSeasonStacker(SpiralDitherFieldPerVisitStacker):
    ...
    def _run(self, simData):
        ...
        for s in np.unique(seasons):
            match = np.where(seasons == s)[0]
            vertexID = vertexID % self.numPoints
            simData["spiralDitherPerSeasonRa"][match] = simData[self.raCol][
                match
            ] + self.xOff[vertexID] / np.cos(simData[self.decCol][match])
            simData["spiralDitherPerSeasonDec"][match] = (
                simData[self.decCol][match] + self.yOff[vertexID]
            )
            vertexID += 1
        ...
        # Wrap into expected range.
        (
            simData["spiralDitherPerSeasonRa"],
