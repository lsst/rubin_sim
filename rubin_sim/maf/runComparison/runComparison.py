...
class RunComparison:
    ...
    * define the directories in which to search for resultsDb (no search for further subdirectories, and
    ...
    def __init__(
        self, baseDir=None, runDirs=None, defaultResultsDb="resultsDb_sqlite.db"
    ):
        ...
        if runDirs is not None:
            ...
            for r in self.runDirs:
                if not (os.path.isfile(os.path.join(r, self.defaultResultsDb))):
                    warnings.warn(
                        f"Could not find resultsDb file {self.defaultResultsDb} in {r}"
                    )
                else:
                    runDirs.append(r)
            self.runDirs = runDirs
            ...
            if self.baseDir is None:
                raise Exception(
                    "Both baseDir and runDirs cannot be None - please specify "
            ...
            self.runDirs = [
                r.replace(f"/{self.defaultResultsDb}", "")
                for r in glob.glob(
                    self.baseDir + "/**/" + self.defaultResultsDb, recursive=True
                )
            ]
        self._connect_to_results()
        # Class attributes to store the stats data:
        self.summaryStats = None  # summary stats
        self.normalizedStats = (
    ...
    def addSummaryStats(self, metricDict=None, verbose=False):
        ...
        for mName, metric in metricDict.items():
            ...
            if self.summaryStats is None:
                ...
                self.summaryStats = self.summaryStats.join(
                    tempStats, how="outer", lsuffix="_x"
                )
