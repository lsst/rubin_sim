...
def OpsimDatabase(
    database, driver="sqlite", host=None, port=None, longstrings=False, verbose=False
):
    ...
    if version == "FBS":
        ...
        warnings.warn(
            "Could not identify opsim database version; just using Database class instead"
        )
        opsdb = Database(
            database,
            driver=driver,
...
class BaseOpsimDatabase(Database):
    ...
    def __init__(
        ...
        super(BaseOpsimDatabase, self).__init__(
            database=database,
            driver=driver,
            host=host,
            port=port,
    ...
    def fetchMetricData(
        self, colnames, sqlconstraint=None, groupBy="default", tableName=None
    ):
        """
        ...
        if groupBy == "default" and tableName != self.defaultTable:
            groupBy = None
        metricdata = super(BaseOpsimDatabase, self).fetchMetricData(
            colnames=colnames,
    ...
    def fetchFieldsFromSummaryTable(
        self, sqlconstraint=None, raColName=None, decColName=None
    ):
        """
    ...
    def fetchOpsimRunName(self):
        ...
        if "Session" not in self.tables:
            ...
            res = self.query_columns(
                "Session", colnames=[self.sessionIdCol, self.sessionHostCol]
            )
            runName = (
                str(res[self.sessionHostCol][0]) + "_" + str(res[self.sessionIdCol][0])
            )
        return runName
    ...
    def fetchTotalSlewN(self):
        ...
        if "SlewActivities" not in self.tables:
            ...
            query = "select count(distinct(%s)) from SlewActivities where %s >0" % (
                self.slewId,
                self.delayCol,
            )
...
class OpsimDatabaseFBS(BaseOpsimDatabase):
    ...
    def fetchPropInfo(self):
        ...
        propTags = {"WFD": [], "DD": [], "NES": []}
        propData = self.query_columns(
            "Proposal",
            colnames=["proposalId", "proposalName", "proposalType"],
            sqlconstraint=None,
        )
        for propId, propName, propType in zip(
            propData["proposalId"], propData["proposalName"], propData["proposalType"]
        ):
    ...
    def createSQLWhere(self, tag, propTags):
        ...
        if (tag not in propTags) or (len(propTags[tag]) == 0):
            ...
            sqlWhere = (
                "("
                + " or ".join(
                    ["proposalId = %d" % (propid) for propid in propTags[tag]]
                )
                + ")"
            )
        return sqlWhere
    ...
    def fetchConfig(self):
        ...
        results = self.query_columns(
            "info", ["Parameter", "Value"], sqlconstraint=f'Parameter like "%ersion%"'
        )
        for r in results:
...
class OpsimDatabaseV4(BaseOpsimDatabase):
    ...
    def __init__(
        ...
        super(OpsimDatabaseV4, self).__init__(
            database=database,
            driver=driver,
            host=host,
            port=port,
    ...
    def fetchFieldsFromFieldTable(self, propId=None, degreesToRadians=True):
        ...
        if propId is not None:
            ...
            fielddata = self.query_arbitrary(
                query,
                dtype=list(
                    zip(
                        [self.fieldIdCol, self.raCol, self.decCol],
                        ["int", "float", "float"],
                    )
                ),
            )
        ...
        if degreesToRadians:
            fielddata[self.raCol] = fielddata[self.raCol] * np.pi / 180.0
            fielddata[self.decCol] = fielddata[self.decCol] * np.pi / 180.0
        return fielddata
    ...
    def fetchPropInfo(self):
        ...
        try:
            propData = self.query_columns(
                "Proposal",
                colnames=[self.propIdCol, self.propNameCol],
                sqlconstraint=None,
            )
        except ValueError:
            propData = []
        ...
        if assignData:
            for propId, propName in zip(
                propData[self.propIdCol], propData[self.propNameCol]
            ):
    ...
    def createSlewConstraint(self, startTime=None, endTime=None):
        ...
        """
        query = (
            "select min(observationStartMJD), max(observationStartMJD) from obsHistory"
        )
        ...
        if startTime > res["mjdMax"][0] or endTime < res["mjdMin"][0]:
            warnings.warn(
                "Times requested do not overlap survey operations (%f to %f)"
                % (res["mjdMin"][0], res["mjdMax"][0])
            )
            return None
        # Find the slew ID matching the start Time.
        query = (
            "select min(observationId) from obsHistory where observationStartMJD >= %s"
            % (startTime)
        )
        res = self.query_arbitrary(query, dtype=[("observationId", "int")])
        obsHistId = res["observationId"][0]
        query = (
            "select slewCount from slewHistory where ObsHistory_observationId = %d"
            % (obsHistId)
        )
        ...
        query = (
            "select max(observationId) from obsHistory where observationStartMJD <= %s"
            % (endTime)
        )
        res = self.query_arbitrary(query, dtype=[("observationId", "int")])
        obsHistId = res["observationId"][0]
    ...
    def fetchRequestedNvisits(self, propId=None):
        ...
        propData = self.query_columns(
            "Proposal",
            colnames=[self.propNameCol, self.propTypeCol],
            sqlconstraint=constraint,
        )
        nvisits = {}
        for f in self.filterlist:
            nvisits[f] = 0
        for pName, propType in zip(
            propData[self.propNameCol], propData[self.propTypeCol]
        ):
            if propType.lower() == "general":
                for f in self.filterlist:
                    constraint = (
                        'paramName="science/general_props/values/%s/filters/%s/num_visits"'
                        % (pName, f)
                    )
                    val = self.query_columns(
                        "Config", colnames=["paramValue"], sqlconstraint=constraint
                    )
                    if len(val) > 0:
        ...
        for subseq in propDict:
            ...
            if "SubSeqNested" in propDict[subseq]:
                for subseqnested in propDict[subseq]["SubSeqNested"]:
                    events = (
                        subevents
                        * propDict[subseq]["SubSeqNested"][subseqnested]["Events"]
                    )
                    subfilters = propDict[subseq]["SubSeqNested"][subseqnested][
                        "Filters"
                    ]
    ...
    def fetchConfig(self):
        ...
        if "Session" not in self.tables:
            warnings.warn(
                "Cannot fetch opsim config info as this is not a full opsim database."
            )
            return {}, {}
        ...
        results = self.query_columns(
            "Session", [self.versionCol, self.sessionDateCol, self.runCommentCol]
        )
        ...
        configSummary["Version"]["OpsimDate"] = configSummary["Version"]["OpsimDate"][
            0:10
        ]
        configSummary["RunInfo"] = {}
        ...
        configSummary["RunInfo"]["keyorder"] = [
            "RunName",
            "RunComment",
            "MinAlt",
            "MaxAlt",
        ...
        propData = self.query_columns(
            "Proposal", [self.propIdCol, self.propNameCol, self.propTypeCol]
        )
        configSummary["Proposals"] = {}
        for propid, propname, proptype in zip(
            propData[self.propIdCol],
            propData[self.propNameCol],
            propData[self.propTypeCol],
        ):
            ...
            propdict["keyorder"] = [
                ...
                "HA max",
                "Time weight",
                "Restart Lost Sequences",
                "Restart Complete Sequences",
                "Filters",
            ]
            ...
            constraint = (
                'paramName like "science/%s_props/values/%s/sky_constraints/max_airmass"'
                % ("%", propname)
            )
            propdict["Airmass max"] = self._queryParam(constraint)
            constraint = (
                'paramName like "science/%s_props/values/%s/scheduling/airmass_bonus"'
                % ("%", propname)
            )
            propdict["Airmass bonus"] = self._queryParam(constraint)
            constraint = (
                'paramName like "science/%s_props/values/%s/scheduling/hour_angle_max"'
                % ("%", propname)
            )
            propdict["HA max"] = self._queryParam(constraint)
            constraint = (
                'paramName like "science/%s_props/values/%s/scheduling/hour_angle_bonus"'
                % ("%", propname)
            )
            propdict["HA bonus"] = self._queryParam(constraint)
            constraint = (
                'paramName like "science/%s_props/values/%s/scheduling/time_weight"'
                % ("%", propname)
            )
            propdict["Time weight"] = self._queryParam(constraint)
            constraint = (
                'paramName like "science/%s_props/values/%s/scheduling/restart_lost_sequences"'
                % ("%", propname)
            )
            propdict["Restart Lost Sequences"] = self._queryParam(constraint)
            constraint = (
                'paramName like "science/%s_props/values/%s/scheduling/restart_complete_sequences"'
                % ("%", propname)
            )
            ...
            for f in self.filterlist:
                ...
                for dk, qk in zip(dictkeys, querykeys):
                    constraint = (
                        'paramName like "science/%s_props/values/%s/filters/%s/%s"'
                        % ("%", propname, f, qk)
                    )
                    propdict["Filters"][f][dk] = self._queryParam(constraint)
                propdict["Filters"][f]["keyorder"] = [
                    "Filter",
                    "MaxSeeing",
                    "MinSky",
                    "MaxSky",
...
class OpsimDatabaseV3(BaseOpsimDatabase):
    def __init__(
        ...
        """
        super(OpsimDatabaseV3, self).__init__(
            database=database,
            driver=driver,
            host=host,
            port=port,
    ...
    def fetchFieldsFromFieldTable(self, propId=None, degreesToRadians=True):
        ...
        if propId is not None:
            query = "select f.%s, f.%s, f.%s from %s as f" % (
                self.fieldIdCol,
                self.raCol,
                self.decCol,
                "Field",
            )
            query += ", %s as p where (p.Field_%s = f.%s) " % (
                "Proposal_Field",
                self.fieldIdCol,
                self.fieldIdCol,
            )
            ...
            fielddata = self.query_columns(
                "Field",
                colnames=[self.fieldIdCol, self.raCol, self.decCol],
                groupBy=self.fieldIdCol,
            )
        if degreesToRadians:
            fielddata[self.raCol] = fielddata[self.raCol] * np.pi / 180.0
    ...
    def fetchPropInfo(self):
        ...
        if "Proposal" not in self.tables:
            ...
            propData = self.query_columns(
                "Proposal",
                colnames=[self.propIdCol, self.propConfCol, self.propNameCol],
                sqlconstraint=None,
            )
            for propid, propname in zip(
                propData[self.propIdCol], propData[self.propConfCol]
            ):
                # Strip '.conf', 'Prop', and path info.
                propIDs[int(propid)] = re.sub(
                    "Prop", "", re.sub(".conf", "", re.sub(".*/", "", propname))
                )
            # Find the 'ScienceType' from the config table, to indicate DD/WFD/Rolling, etc.
            sciencetypes = self.query_columns(
                "Config",
                colnames=["paramValue", "nonPropID"],
    ...
    def createSlewConstraint(self, startTime=None, endTime=None):
        ...
        res = self.query_arbitrary(query, dtype=[("observationId", "int")])
        obsHistId = res["observationId"][0]
        query = "select slewID from slewHistory where ObsHistory_obsHistID = %d" % (
            obsHistId
        )
        res = self.query_arbitrary(query, dtype=[("slewCount", "int")])
        minSlewCount = res["slewCount"][0]
    ...
    def createSQLWhere(self, tag, propTags):
        ...
        if (tag not in propTags) or (len(propTags[tag]) == 0):
            ...
            sqlWhere = (
                "("
                + " or ".join(["propID = %d" % (propid) for propid in propTags[tag]])
                + ")"
            )
        return sqlWhere
    ...
    def fetchRequestedNvisits(self, propId=None):
        ...
        for pId, propType in zip(propData[self.propIdCol], propData[self.propNameCol]):
            perPropConfig = self.query_columns(
                "Config",
                colnames=["paramName", "paramValue"],
                sqlconstraint='nonPropID = %d and paramName!="userRegion"' % (pId),
            )
            ...
            if propType == "WL":
                ...
                nvisits = np.array(
                    self._matchParamNameValue(perPropConfig, "Filter_Visits"), int
                )
            elif propType == "WLTSS":
    ...
    def fetchConfig(self):
        ...
        query = "select %s, %s, %s from Proposal group by %s" % (
            self.propIdCol,
            self.propConfCol,
            self.propNameCol,
            self.propIdCol,
        )
        propdata = self.query_arbitrary(
            query,
            dtype=(
                [
                    (self.propIdCol, int),
        ...
        propnames = np.array(
            [
                os.path.split(x)[1].replace(".conf", "")
                for x in propdata[self.propConfCol]
            ]
        )
        # Get 'nice' module names
        moduledata = self.query_columns(
            "Config",
            colnames=[
                "moduleName",
            ],
            sqlconstraint="nonPropID=0",
        )
        modulenames = np.array(
            [os.path.split(x)[1].replace(".conf", "") for x in moduledata["moduleName"]]
        )
        ...
        for longmodname, modname in zip(moduledata["moduleName"], modulenames):
            config[modname] = self.query_columns(
                "Config", colnames=cols, sqlconstraint='moduleName="%s"' % (longmodname)
            )
            config[modname] = config[modname][["paramName", "paramValue", "comment"]]
        for propid, propname in zip(propdata[self.propIdCol], propnames):
            config[propname] = self.query_columns(
                "Config",
                colnames=cols,
                sqlconstraint='nonPropID="%s" and paramName!="userRegion"' % (propid),
            )
        ...
        for propid, propname in zip(propdata[self.propIdCol], propnames):
            ...
            propdict["PropType"] = propdata[self.propNameCol][
                np.where(propnames == propname)
            ]
            propdict["RelPriority"] = self._matchParamNameValue(
                config[propname], "RelativeProposalPriority"
            )
            ...
            propdict["PerFilter"] = {}
            for key, keyword in zip(
                ["Filters", "MaxSeeing", "MinSky", "MaxSky"],
            ...
            exptime = float(
                self._matchParamNameValue(config[propname], "ExposureTime")[0]
            )
            temp = self._matchParamNameValue(config[propname], "Filter_ExpFactor")
            if len(temp) > 0:
                propdict["PerFilter"]["VisitTime"] = temp * exptime
            else:
                propdict["PerFilter"]["VisitTime"] = np.ones(
                    len(propdict["PerFilter"]["Filters"]), float
                )
            ...
            temp = self._matchParamNameValue(
                config[propname], "RestartCompleteSequences"
            )
            if len(temp) > 0:
                if temp[0] == "True":
            ...
            if propdict["PropType"] == "WL":
                ...
                propdict["PerFilter"]["NumVisits"] = np.array(
                    self._matchParamNameValue(config[propname], "Filter_Visits"), int
                )
