...
class SdssDatabase(Database):
    ...
    def fetchMetricData(
        self, colnames, sqlconstraint, groupBy=None, cleanNaNs=True, **kwargs
    ):
        """Get data for metric"""
        ...
        if sqlconstraint != sqlconstraint.replace('"', "'"):
            warnings.warn(
                "Warning:  Replacing double quotes with single quotes in SQL where-clause. \
                           Double quotes are not defined in standard SQL."
            )
            sqlconstraint = sqlconstraint.replace('"', "'")
        data = table.query_columns_Array(
