...
date --iso=s
time ${PRENIGHT_SIM} --scheduler ${TELESCOPE}.pickle.xz --opsim previous_visits.db --repo=${TS_CONFIG_OCS_REPO} --script ${SCHEDULER_CONFIG_SCRIPT} --config_version ${TS_CONFIG_OCS_VERSION} --telescope ${TELESCOPE} 2>&1 > ${WORK_DIR}/prenight_sim.out
date --iso=s
