...
class TestChebyValues(unittest.TestCase):
    def setUp(self):
        ...
        self.chebyFits.write(
            self.coeffFile, self.residFile, self.failedFile, append=False
        )
        self.coeffKeys = [
            "objId",
            "tStart",
        ...
        for k in chebyValues.coeffs:
            if k == "objId":
                ...
                np.testing.assert_allclose(
                    chebyValues.coeffs[k], chebyValues2.coeffs[k], rtol=0, atol=1e-5
                )
    ...
    def testGetEphemerides(self):
        ...
        ephemerides = chebyValues.getEphemerides(
            self.tStart + self.interval * 2, objIds, extrapolate=False
        )
        self.assertTrue(
...
class TestJPLValues(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.jpl = pd.read_table(
            os.path.join(self.jplDir, "807_n747.txt"), delim_whitespace=True
        )
