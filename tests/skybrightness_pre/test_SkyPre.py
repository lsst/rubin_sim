...
class TestSkyPre(unittest.TestCase):
    ...
    def testReturnMags(self):
        ...
        if self.data_present:
            ...
            mjds.append(
                sms[0].info["mjds"][between + 1] + sms[0].header["timestep_max"]
            )
    ...
    def testSunMoon(self):
        ...
        if self.data_present:
            ...
            location = EarthLocation(
                lat=telescope.latitude_rad * u.rad,
            ...
            for mjd in mjds:
                ...
                sun_dist = _angularSeparation(
                    sun.ra.rad, sun.dec.rad, pre_calced["sunRA"], pre_calced["sunDec"]
                )
                assert sun_dist < dist_tol

                self.assertLess(
                    np.abs(pre_calced["moonAlt"] - moon_aa.alt.rad), dist_tol
                )
                moon_dist = _angularSeparation(
                    moon.ra.rad,
                    moon.dec.rad,
                    pre_calced["moonRA"],
                    pre_calced["moonDec"],
        ...
        if self.data_present:
            ...
            for mjd in mjds:
                ...
                if original_model.sunAlt < np.radians(-12.0):
                    ...
                    good_am = np.where((am1 >= 1.0) & (am1 <= am_limit))
    ...
    def test_various(self):
        ...
        sm = sbp.SkyModelPre(load_length=3)
